...
function _load_controller()
   ...
   if not IsPluginInstalled(theme_controller_ID) then
      local inner_action =
      [[if not theme_plugin_loading then theme_plugin_loading = true; DoAfterSpecial(0.1, 'require \'checkplugin\'; theme_plugin_loading = nil; do_plugin_check_now(\']] ..
      theme_controller_ID .. [[\', \']] .. theme_controller_name .. [[\')', sendto.scriptafteromit) end]]
...
function AddButton(win, button_id, font, left, top, text, utf8, x_padding, y_padding, tooltip, mousedown_callback,
                   mouseup_callback, width, height, style)
   local right, bottom
   ...
   Theme.button_metrics[button_id] = { win, button_id, left, top, width, height, style, text, font, utf8, x_padding,
      y_padding }
   ...
   if (style == Theme.STYLE_3D) then
      ...
      right, bottom = DrawTextBox(win, font, left, top, text, utf8, true, bgcolor, textcolor, x_padding, y_padding, width,
         height)
   end
   if WindowMoveHotspot(win, button_id, left, top, right, bottom) ~= 0 then
      WindowAddHotspot(win, button_id, left, top, right, bottom, nil, nil, "Theme.TextButtonMouseDown",
         "Theme.TextButtonMouseCancel", "Theme.TextButtonMouseUp", tooltip, 1, 0)
   end
   return right, bottom
end
...
function Add3DTextButton(win, button_id, font, left, top, text, utf8, x_padding, y_padding, tooltip, mousedown_callback,
                         mouseup_callback, width, height)
   return AddButton(win, button_id, font, left, top, text, utf8, x_padding, y_padding, tooltip, mousedown_callback,
      mouseup_callback, width, height, true)
end

function TextButtonMouseDown(flags, hotspot_id)
   local callbacks = Theme.button_callbacks[hotspot_id]
   local win, button_id, left, top, width, height, style, text, font, utf8, x_padding, y_padding = unpack_hotspot_metrics(
   hotspot_id)
