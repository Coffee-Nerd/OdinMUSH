...
function TextRect:drawLine(line, styles, selection_start, selection_end, utf8, show_bold)
   ...
   for _, v in ipairs(styles) do
      ...
      if v.backcolour and (v.backcolour ~= 0) then
         ...
         if (selection_start ~= nil) and (selection_end ~= nil) then
            if (bgleft <= selection_start) and (bgright >= selection_end) then
               WindowRectOp(self.window, 2, bgleft, top, math.min(self.padded_right, selection_start),
                  math.min(self.padded_bottom, top + self.line_height), v.backcolour)
         ...
         if bgleft < bgright then
            WindowRectOp(self.window, 2, bgleft, top, math.min(self.padded_right, bgright),
               math.min(self.padded_bottom, top + self.line_height), v.backcolour)
         end
      end
      ...
      -- foreground
      left = left +
      WindowText(self.window, font, t, left, top, self.padded_right, self.padded_bottom, v.textcolour, is_utf)
   end
end
      ...
      for count = self.display_start_line, self.display_end_line do
         ...
         if self.keepscrolling == "" then
            ...
            for _, url_part in ipairs(self.wrapped_lines[count][3]) do
               ...
               local left = self.padded_left +
               self:styles_width(TruncateStyles(line_styles, 0, url_part.start - 1), show_bold)
               ...
               local right = left +
               self:styles_width(TruncateStyles(line_styles, url_part.start, url_part.stop), show_bold)
               local top = self.padded_top + ((count - self.display_start_line) * self.line_height) - 1
               local bottom = top + self.line_height
               local link_id = self:generateHotspotID(table.concat({ url_part.text, " ", count, " ", url_part.start, " ",
                  url_part.stop }))

               if not WindowHotspotInfo(self.window, link_id, 1) then
                  self.hyperlinks[link_id] = url_part.text
                  WindowAddHotspot(self.window, link_id, left, top, math.min(right, self.padded_right), bottom,
                     "TextRect.linkHover", "TextRect.cancelLinkHover", "TextRect.clickUrl", "", "TextRect.mouseUp",
                     url_part.label or ("Right-click this URL if you want to open it:\n" .. url_part.text), 1)
...
function TextRect:draw_scrollback_marker()
   if self.display_end_line < self.num_wrapped_lines then
      ...
      WindowCircleOp(self.window, 2, left, top, self.right - 5, self.bottom + 2, Theme.THREE_D_SOFTSHADOW, 0, 1,
         Theme.THREE_D_GRADIENT_FIRST, 0)
      WindowText(self.window, self.font, text, left + 5, top, self.right - 3, self.bottom, Theme.THREE_D_SURFACE_DETAIL,
         false)
   end
end
...
function TextRect:initArea()
   ...
   if (not self.unselectable) or self.scrollable or self.external_scroll_handler then
      if self.unselectable then
         WindowAddHotspot(self.window, self.area_hotspot, self.left, self.top, self.right, self.top + self.height, "", "",
            "", "", "TextRect.mouseUp", "", nil, 0)
      else
         WindowAddHotspot(self.window, self.area_hotspot, self.left, self.top, self.right, self.top + self.height, "", "",
            "TextRect.mouseDown", "TextRect.cancelMouseDown", "TextRect.mouseUp", "", miniwin.cursor_ibeam, 0)
...
function TextRect.mouseDown(flags, hotspot_id)
   ...
   if tr.last_click_time and ((now - tr.last_click_time) < 0.4) then
      ...
      local start_pos, end_pos, _, _ = tr:get_target_bounds(tr.separator_pattern, tr.temp_start_line,
         tr.temp_start_copying_x, "start")
   ...
   if tr.call_on_select then
      tr.call_on_select(tr.copy_start_line, tr.copy_end_line, tr.start_copying_pos, tr.end_copying_pos,
         tr.start_copying_x, tr.end_copying_x)
   end
   CallPlugin("abc1a0944ae4af7586ce88dc", "BufferedRepaint")
end
...
function TextRect:updateSelect()
   ...
   local start_pos, _, start_x, _ = self:get_target_bounds(self.separator_pattern, self.copy_start_line,
      self.start_copying_x, "start")
   self.start_copying_pos = start_pos
   self.start_copying_x = start_x
   local _, end_pos, _, end_x = self:get_target_bounds(self.separator_pattern, self.copy_end_line, self.end_copying_x,
      "end")
   self.end_copying_pos = end_pos
   ...
   if self.call_on_select then
      self.call_on_select(self.copy_start_line, self.copy_end_line, self.start_copying_pos, self.end_copying_pos,
         self.start_copying_x, self.end_copying_x)
   end
end
...
function TextRect:rightClickMenu(hotspot_id)
   ...
   if self.external_menu_generator then
      local external_string, external_functions = self.external_menu_generator((self.copy_start_line ~= nil) and
      (self.copy_end_line ~= nil) and self:selected_text(false) or nil)
